...
public class SyntaxProblemReporter {
	...
	public void checkAndApply(IDocument document, int offset, int length, IResource resource) throws CoreException, BadLocationException {
		...
		for (IMarker marker: markers) {
			...
			if (fullScan || markerLineNr == regionLineNr) markersToDelete.add(marker);
		}
	...
	public SyntaxProblem[] check(IDocument document, int offset, int length) throws CoreException, BadLocationException {
		...
		for (int lineNr=startingLineNr; lineNr<startingLineNr+lineCount; lineNr++) {
			...
			if (lineNr+startingLineNr+1<lineCount) lineEnd = document.getLineOffset(lineNr+1);
			...
			if (matchingInstruction == null) {
				...
				problems.add(new SyntaxProblem(IMarker.SEVERITY_ERROR, lineOffset, firstWord.length(), "Unknown instruction: " + firstWord));
			} else if (!matchingInstruction.equals(firstWord)) {
				// Warning: case incorrect
				problems.add(new SyntaxProblem(IMarker.SEVERITY_WARNING, lineOffset, firstWord.length(), "Instructions should be upper case"));
			}
		}
